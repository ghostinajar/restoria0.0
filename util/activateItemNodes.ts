...
async function activateItemNodes(itemNodes : Array<IItemNode>, inventory : Array<IItem>, isNested = false) {
    for (const itemNode of itemNodes) {
        try {
            ...
            const blueprint : any = await zone.itemBlueprints.find((blueprint : any) => blueprint._id.toString() === itemNode.loadsItemBlueprintId.toString());        
            if(!blueprint) {
            ...
            for(let i = 0; i < itemNode.quantity; i++) {
                ...
                if (item.itemType == 'container' && !isNested) {
                    ...
                    logger.debug(`Items in container "${item.name}": ${item.inventory.map((item : any) => {return item.name})}`);

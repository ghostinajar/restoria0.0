...
const HELP: {
  ...
  CREATE_ITEM: [
    ...
    `<span style="color:var(--red)">Container:</span> if checked, item may spawn or hold other items inside`,
  ...
  CREATE_ZONE: [
    `${padStringToLength79(`CREATE ZONE`)}`,
  ...
  EDIT_ZONE: [
    ...
    `<span style="color:var(--red)">Study:</span> The player learns some history when they study the zone.`,
    `<span style="color:var(--red)">Research:</span> These paragraphs tell the full history of the zone.`,
    `  A good research text rewards the reader with some secrets about the zone. Read HELP DESCRIPTION for more.`,
  ],
  EDITOR: [
  ...
  ERASE: [
    ...
    `Use the ERASE or DELETE command to ERASE ITEM, ERASE MOB, or ERASE ROOM in your zone. To erase a user, contact Ralu or another game administrator. Except in special circumstances, we'll only erase one of your users per month. You can't erase a zone. Edit or erase its contents instead. Remember, <span style="color:var(--red)">ERASE cannot be undone!</span> We recommend saving all your writing somewhere, (e.g. Google Drive), so you have a back-up copy of your hard work. Some wise authors do all their writing in another doc, then copy/paste into Restoria. Why not back up your writing before you ERASE, just in case?`,
  ],
  ERASE_EXIT: [
  ...
  ERASE_USER: [
    `${padStringToLength79(`ERASE USER`)}`,
  ...
  ERASE_ZONE: [
    `${padStringToLength79(`ERASE ZONE`)}`,

export const validCommandWords = ["author", "auth", "character", "char", "say", "shout", "t", "tel", "telepath", "tell", "who"];
export const commandsWith1Param = [
"abbreviate", "affects", "afk", "ambush", "appraise", "ask", 
"assist", "attack", "autoassist", "autoflee", "backstab", 
"bash", "block", "brief", "bs", "character", "char", "close", "consider", 
"count", "delete", "deposit", "disarm", "donate", "drink", 
"drop", "eat", "edit", "enter", "examine", "fight", "follow", 
"guildenroll", "guildremove", "help", "ignore", "invite", 
"kick", "kill", "knock", "learn", "leader", "locate", "look", 
"message", "mount", "offer", "open", "order", "partylock", 
"pick", "price", "read", "recycle", "repair", "rescue", "sayto", 
"score", "search", "sell", "setjunk", "setparty", "setphase", 
"setrescue", "setsummon", "solo", "sort", "split", "store", 
"study", "target", "t", "tel", "telepath", "tell", "tell", "throw", "track", "train", 
"trip", "unparty", "wake", "where", "whisper", "wield", 
"withdraw", "zoneinfo", "zonelist"
]
export const commandsWith2Params = [
"buy", "cast", "compare", "create", "dub", "duel", "empty", 
"equip", "fill", "fly", "freeze", "get", "give", "hand", "mode", 
"pour", "put", "reposition", "rename", "send", "steal", "take", 
"transfer", "use", "wear"
]
    
export default validCommandWords;
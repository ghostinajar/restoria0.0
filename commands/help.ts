...
function help(parsedCommand: IParsedCommand, user: IUser) {
  try {
    ...
    if (!target) {
      ...
      messageToUsername(
        user.username,
        `Here are all the commands you can try in Restoria:`,
        `help`,
        true
      );
      ...
      messageToUsername(
        user.username,
        `What would you like help with? E.g. HELP CREATE`,
        `help`,
        true
      );
      return;
    ...
    if (!HELP.hasOwnProperty(target.toUpperCase())) {
      ...
      messageToUsername(
        user.username,
        `Sorry, there's no help available yet for ${target}. Try HELP on its own.`,
        `help`
      );
      return;
    }

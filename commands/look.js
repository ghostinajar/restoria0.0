...
function lookTarget(target, room, user, lookArray) {
    ...
    if (targetObject) {
        ...
        if (targetObject.description.examine) {
            lookArray.push(makeMessage(true, `userDescription`, `${targetObject.description.examine}`));
        }
    ...
    if (targetObject) {
        ...
        // push a message for the target.description.look into lookArray
        lookArray.push(makeMessage(true, `mobDescription`, `${targetObject.description.examine}`));
        pushTargetEquipped(targetObject, lookArray);
        pushTargetInventory(targetObject, lookArray);
        return;
    }
    ...
    if (targetObject) {
        ...
        // push a message for the target.description.look into lookArray
        lookArray.push(makeMessage(true, `itemDescription`, `${targetObject.description.examine}`));
        if (targetObject.itemType === 'container' || targetObject.itemType === 'liquid_container') {
            pushTargetInventory(targetObject, lookArray);
        }
        return;
    }
    // If no target found, push a message saying target not found
    lookArray.push(makeMessage(false, `targetNotFound`, `No '${target}' found in the room.`));
}
function lookRoom(room, user, lookArray) {
    ...
    let roomDescriptionMessage = makeMessage(true, `roomDescription`, `${room.description.look}`);

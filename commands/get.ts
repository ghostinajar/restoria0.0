...
async function get(parsedCommand: IParsedCommand, user: IUser) {
  try {
    ...
    if (parsedCommand.indirectObject) {
      ...
      console.log(
        `container specified, (${parsedCommand.indirectObject}) checking user inventory...`
      );
      let specifiedContainer = await findObjectInInventory(
        user.inventory,
      ...
      if (specifiedContainer) {
        ...
        console.log(
          `container found in user inventory! ${specifiedContainer.name}`
        );
        ...
        specifiedContainer = await findObjectInInventory(
          room.inventory,
          parsedCommand.indirectObject,
          parsedCommand.indirectObjectOrdinal
        );
        if (specifiedContainer) {
          ...
          if (room.mobs.length > 0) {
            ...
            if (eligibleMobs.length > 0) {
              messageToUsername(
                user.username,
                `You can't GET items from mobs. Maybe a rogue could STEAL it...`,
                `help`,
                true
              );
              return;
            ...
            messageToUsername(
              user.username,
              `You can't find the ${parsedCommand.indirectObject} to get from.`,
              `rejection`
            );
            return;
          }
    ...
    if (parsedCommand.targetsAll) {
      ...
      let targetItems = originInventory.filter((item) =>
        item.keywords.some(
          (keyword) =>
      ...
      let targetObject = await findObjectInInventory(
        originInventory,
        parsedCommand.directObject,
      ...
      console.log(
        `moving single matching item ${targetObject.name} from originInventory to user.inventory`
      );
      await relocateItem(targetObject, user.inventory, originInventory);
      messageToUsername(
        user.username,
        `You got ${targetObject.name}.`,
        "success"
      );
    }

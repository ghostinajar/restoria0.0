...
async function eraseZone(zoneId: string, user: IUser) {
  try {
    ...
    if (!user.isAdmin) {
      logger.warn(
        `User ${user._id} (${user.name} submitted erase_zone_form successfully on zone id ${zoneId}. How?)`
      );
      worldEmitter.emit(
    ...
    if (!mongoose.Types.ObjectId.isValid(zoneId)) {
      ...
      worldEmitter.emit(
        `messageFor${user.username}`,
        makeMessage(`rejection`, `Sorry, that's an invalid id.`)
      );
      return;
    }
    ...
    if (!zone) {
      ...
      worldEmitter.emit(
        `messageFor${user.username}`,
        makeMessage(`rejection`, `Zone not found.`)
      );
      return;
    }

    // Empty zone of users
    let zoneEmptiedOfUsers: boolean = await new Promise((resolve) => {
    ...
    if (!zoneEmptiedOfUsers) {
      ...
      worldEmitter.emit(
        `messageFor${user.username}`,
        makeMessage(`rejection`, `Failed to safely remove all users from zone.`)
      );
      return;
    }

    // Close zone
    let zoneClosed: boolean = await new Promise((resolve) => {
    ...
    if (!zoneClosed) {
      ...
      worldEmitter.emit(
        `messageFor${user.username}`,
        makeMessage(`rejection`, `Failed to safely close zone.`)
      );
      return;
    }
    ...
    try {
      ...
      worldEmitter.emit(
        `messageFor${user.username}`,
        makeMessage(`rejection`, `Database error occurred while deleting zone.`)
      );
      return;
    }
    ...
    worldEmitter.emit(
      `messageFor${user.username}`,
      makeMessage(`success`, `You successfully erased zone ${zoneId}`)
    );

  ...
  try {
    ...
    if (!parsedCommand.string) {
      worldEmitter.emit(
        ...
        makeMessage(`rejection`, `Please enter new password twice (e.g. "PASSWORD new14Password new14Password").`)
      );
      return;
    }
    ...
    if (passArray[0] !== passArray[1]) {
      ...
      worldEmitter.emit(
        `messageFor${user.username}`,
        makeMessage(`rejection`, `Please enter new password twice (e.g. "PASSWORD waTerm3lon waTerm3lon").`)
      );
      return;
    }
    ...
    if (!passArray[0].match(passwordRequirements)) {
      ...
      worldEmitter.emit(
        `messageFor${user.username}`,
        makeMessage(`rejection`, `Password must be at least 8 characters long and contain at least one number, one lowercase letter, and one uppercase letter.`)
      );
      return;
    }
    ...
    if (!encryptedPassword) {
      ...
      worldEmitter.emit(
        `messageFor${user.username}`,
        makeMessage(`rejection`, `Sorry, we failed to encrypt your password. We logged the bug, and Ralu will try to fix it ASAP.`)
      );
      return;
    }

...
async function put(parsedCommand, user) {
    try {
        ...
        let container = selectTargetByOrdinal(parsedCommand.indirectObjectOrdinal || 0, containerKeyword, user.inventory);
        if (!container) {
        ...
        if (!container.inventory) {
            messageToUsername(user.username, `You can't put items in ${container.name}.`, "failure");
            return;
        }
        if (container.inventory.length >= container.capacity) {
            messageToUsername(user.username, `There's no room to put items in ${container.name}.`, "failure");
            return;
        }
        if (parsedCommand.targetsAll) {
            // handle "all"
            let itemsToPut = user.inventory.filter((item) => item.keywords.some((keyword) => keyword.startsWith(itemKeyword)));
            if (!itemsToPut) {
            ...
            let itemToPut = await findObjectInInventory(user.inventory, itemKeyword, parsedCommand.directObjectOrdinal);
            if (!itemToPut) {
                messageMissingTargetToUser(user, itemKeyword);
                return;
            }
...
async function handleMissingKeyword(username) {

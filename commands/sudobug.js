...
async function sudobug(parsedCommand, user) {
    try {
        if (!user?.isAdmin) {
        ...
        if (!parsedCommand.string) {
            messageToUsername(user?.username, `Please provide a bug ID and values, e.g. "SUDOBUG 1234 valid fixed".`, "failure", true);
            return;
        }
        ...
        if (!bug) {
            messageToUsername(user?.username, `Bug ${bugId} not found.`, "failure", true);
            return;
        }
        bug.isValid = valid;
        bug.isFixed = fixed;

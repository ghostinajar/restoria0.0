...
async function bug(bugDescription, user) {
    try {
        let newBugData = {
        ...
        await newBug.save();
        logger.info(`Author "${user.name}" reported a bug: "${newBug.description}".`);
        worldEmitter.emit(`messageFor${user.username}`, makeMessage("success", `Thank you for your bug report! Ralu will review it soon.`));
    }
    catch (error) {

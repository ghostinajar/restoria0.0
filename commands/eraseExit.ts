...
async function eraseExit(direction: string, user: IUser) {
  try {
    ...
    if (!destinationRoom) {
      throw new Error(
        `room not found to the ${direction} of room ${originRoom._id}`
      );
    }
    ...
    worldEmitter.emit(
      ...
      makeMessage("success", `You erased the exit to the ${direction}.`)
    );

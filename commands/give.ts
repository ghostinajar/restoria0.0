...
async function give(parsedCommand: IParsedCommand, user: IUser) {
  try {
    ...
    if (!recipientKeyword) {
      messageToUsername(
        user.username,
        `Give to whom? e.g. GIVE APPLE RALU`,
        `help`,
        true
      );
      return;
    ...
    if (!item) {
      messageToUsername(
        user.username,
        `You don't have the ${itemKeyword} in your inventory.`,
        `help`,
        false
      );
      return;
    ...
    let recipient;
    recipient = room.users.find((user) =>
      user.username.startsWith(recipientKeyword.toLowerCase())
    );
    if (!recipient) {

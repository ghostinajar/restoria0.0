...
async function move(parsedCommand: IParsedCommand, agent: IUser) {
  ...
  switch (requestedDirection) {
    case `n`: { requestedDirection = `north`; break; }
    case `e`: { requestedDirection = `east`; break; }
    case `s`: { requestedDirection = `south`; break; }
    case `w`: { requestedDirection = `west`; break; }
    case `u`: { requestedDirection = `up`; break; }
    case `d`: { requestedDirection = `down`; break; }
    default: break;
  }
...
// get destination Room
  const destinationRoom: IRoom = await new Promise((resolve) => {
    worldEmitter.once(
      `zoneManagerReturningRoom${originRoom.exits[direction].destinationLocation.inRoom.toString()}`,
      resolve
    );
 ...
 // Alert destination room to add user to users or mobs array
  ...
  if (agent.username) {
    await worldEmitter.emit(
      `messageFor${agent.username}`,
      makeMessage(`userMove`, `You move ${requestedDirection}.`)
    );
  };
  await worldEmitter.emit(
    `messageFor${agent.username}sRoom`,
    makeMessage(`userMove`, `${agent.name} has arrived.`)
  );
  if (agent.username) {look({commandWord: `look`}, agent);};
}

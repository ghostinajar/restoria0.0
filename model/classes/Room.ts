...
roomSchema.methods.addEntityTo = function(entityType : string, instance : IMob | IItem | IUser) {
    //if the array exists, and the instance doesn't already exist in the array, add it
    if (this[entityType] && !this[entityType].find((el : IMob | IItem | IUser) => el._id.toString() === instance._id.toString())) {
        this[entityType].push(instance);
    }
};

roomSchema.methods.removeEntityFrom = function(entityType : string, instance : IMob | IItem | IUser) {
    if (this[entityType]) {
      this[entityType] = this[entityType].filter(function(entity : IMob | IItem | IUser) {
